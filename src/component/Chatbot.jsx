...
export default function Chatbot({ showChat, setShowChat }) {
  ...
  const generateBotResponse = async (history) => {
    const updateHistory = (text, isError = false) => {
      setChatHistory((prevState) => [
        ...prevState.filter((msg) => msg.text !== "Thinking..."),
    ...
    const userChat =
      history.filter((msg) => msg.role === "user").pop()?.text || "";
    ...
    try {
      ...
      const apiTextResponse = result.data[0]
        .replace(/\*\*(.*?)\*\*/g, "$1")
        .trim();
  ...
  return (
    <div className={`container ${showChat ? "showChat" : ""}`}>
      <span className={`flow-text textPresetL ${showChat && "hideChat"}`}>
        Get To Know Me
      </span>
      <button
        id="chatbot-toggle"
